...
void main(){
	...
	tes_normal = texture(u_normalMap, tes_texCoord).rgb;

	
	float height = texture(u_heightMap, tes_texCoord).r;
	tes_fragmentPos.y += height * tes_scale;
	
	//tes_fragment in world space, convert to slip space here instead of doing it in the vertex shader
